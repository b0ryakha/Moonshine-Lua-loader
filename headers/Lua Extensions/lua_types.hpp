...
class LuaTable {
private:
    std::map<std::string, std::variant<lua_Number, lua_CFunction, std::string, LuaBoolean, LuaNil>> elements;

public:
    LuaTable(lua_State* lua_state, int index);

    template<typename T>
    T get(const std::string& key) { throw_error("Unknown Type!"); }
    ...
    template<>
    size_t get<size_t>(const std::string& key) {
    ...
    template<>
    LuaNil get<LuaNil>(const std::string& key) {
