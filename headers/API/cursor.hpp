...
namespace lua
{
    ...
    static int cursor_set_pos(lua_State* L) {
        ...
        if (is_regarding_window) {
            if (x < window.getPosition().x) x = window.getPosition().x;
            if (x > window.getPosition().x + window.getSize().x) x = window.getPosition().x + window.getSize().x;
            if (y < window.getPosition().y) y = window.getPosition().y;
            if (y > window.getPosition().y + window.getSize().y) y = window.getPosition().y + window.getSize().y;
        }
    ...
    static int cursor_in_bounds(lua_State* L) {
        ...
        int m_x = sf::Mouse::getPosition().x - window.getPosition().x;
        int m_y = sf::Mouse::getPosition().y - window.getPosition().y;

        if (m_x < 0) m_x = 0;
        if (m_x > window.getPosition().x + window.getSize().x)
            m_x = window.getPosition().x + window.getSize().x;

        if (m_y < 0) m_y = 0;
        if (m_y > window.getPosition().y + window.getSize().y)
            m_y = window.getPosition().y + window.getSize().y;
